...
RSpec.describe Hyrax::Workflow::BatchUnpublishNotification do
  ...
  it "has a subject" do
    expect(notification.subject).to eq("Unpublished Batch #{batch.id}")
  end
  it "has a message" do
    expect(notification.message).to match(/http/)
    expect(notification.message).to match(/Unpublished Batch #{batch.id}/)
  end
end
