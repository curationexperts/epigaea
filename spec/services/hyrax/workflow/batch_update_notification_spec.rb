...
RSpec.describe Hyrax::Workflow::BatchUpdateNotification, :workflow do
  ...
  it "has a subject" do
    expect(notification.subject).to eq("Metadata Update Batch #{batch.id}")
  end
  it "has a message" do
    expect(notification.message).to match(/http/)
    expect(notification.message).to match(/Metadata Update Batch #{batch.id}/)
  end
end
